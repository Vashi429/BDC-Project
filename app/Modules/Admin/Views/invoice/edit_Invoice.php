				...
				<div class="card-body">
					<form action="javascript:void(0)" enctype="multipart/form-data" method="POST" id="edit_invoice_form">
						...
						<div class="form-group row">
							...
							<div class="col-sm-9 col-lg-4">
								<input type="date" name="var_invoice_date" id="var_invoice_date" class="form-control" value="<?= $data['var_invoice_date'] ?>" required max="<?= date('Y-m-d') ?>">
							</div>
						</div>
						<div class="form-group row">
												...
												foreach($data['invoice_item'] as  $ii_key => $ii_value){ 
													...
													<tr class="addmoreInvoice" id="InvoiceItem<?= $ii_value['int_glcode']?>">
														<td>
															...
															<input class="form-control" type="text" required id="var_hsn<?= $ii_key ?>" name="var_hsn[]" class="hsn-small" placeholder="Search HSN Here" oninput="searchhsn('var_hsn<?= $ii_key ?>')" value="<?= $ii_value['var_hsn'] ?>">
...
<script>
		...
		$('#edit_invoice_form').on('submit', function (e) {
			e.preventDefault();
			id = '<?= $data['int_glcode'] ?>';
			var var_paid_amount = $("#var_paid_amount").val();
			var var_final_amount = $("#var_final_amount").val();
			if(parseFloat(var_final_amount) > 0){
				if(parseFloat(var_final_amount) < parseFloat(var_paid_amount)){
					iziToast.error({
						title: 'Error',
						message: 'Invoice due amount is not less then '+CURRENCY_ICON+' '+var_paid_amount+'.',
						position: 'topRight'
					});
				}else{
					$.ajax({
