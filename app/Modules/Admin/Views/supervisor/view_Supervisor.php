            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-header-form">
                        <a class="btn btn-primary text-right mr-2" href="<?= base_url() ?>/admin/supervisor/exportCSV">Export Supervisors</a>
                        <a class="btn btn-primary text-right" href="<?= base_url() ?>/admin/supervisor/addSupervisor">Add Supervisor</a>
                    </div>
                </div>
                        ...
                        <table class="table table-striped" id="viewSupervisor">
                            <thead>
                                <tr>
                                    ...
                                    <th><a href="javascript:void(0);" field="var_email" class="_sort">Email Id<a></th>
                                ...
                                <?php if(!empty($data)){
                                    foreach ($data as $key => $value) { 
                                        ...
                                        <tr id="<?= $value['int_glcode'] ?>">
                                            ...
                                            <td><a href="<?= base_url() ?>/admin/supervisor/editSupervisor/<?= base64_encode($value['int_glcode']) ?>"><?=$value['int_glcode']?></a></td>
                                            <td><?=$value['var_name']?></td>
                                            <td><?=$value['var_username']?></td>
                                            <td><?=$value['var_email']?></td>
                                            <td><?=$value['var_phone']?></td>
                                            <td class="text-center">
                                                <a href="javascript:void(0);">
                                                    <img id="tick-<?php echo $value['int_glcode']; ?>" height="16" width="16" alt="<?php echo $title; ?>" title="<?php echo $title; ?>" src="<?php echo base_url() . '/public/assets/site_imges/' . $tickimg; ?>" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('supervisor', 'mst_supervisor', 'chr_publish', '<?php echo $update_val; ?>', '<?php echo $value['int_glcode']; ?>');">
                                                </a>
                                            </td>
                                        </tr>
                                    <?php }
...
<script src= "<?= base_url() ?>/public/assets/dist/js/supervisor.js"></script>
