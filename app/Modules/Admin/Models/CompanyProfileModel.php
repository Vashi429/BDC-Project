...
class CompanyProfileModel extends Model{
    ...
    public function getData($rowno, $rowperpage, $filter="", $_field='int_glcode', $_sort="desc"){
        $builder = $this->con->table('mst_company_profile');
        $builder->select('*');
    ...
    public function addRecord(){
        ...
        if ($_FILES['var_gst_certi']['name'] != '') {
            if (!is_dir('uploads/company_profile')) {
				mkdir('uploads/company_profile', 0777, TRUE);
			}
			$gst_certi = uniqid() . '-' . $_FILES['var_gst_certi']['name'];
			$gst_certi = preg_replace('/[^a-zA-Z0-9.\-]/s', '', $gst_certi);
			$destination = 'uploads/company_profile/';
        ...
        $this->con->table('mst_company_profile')->insert($userData);
